...
function index_init($http){
    ...
    vm.GoToPayment = user_slot_id =>
    {
        if(typeof(Storage) !== "undefined") {
            sessionStorage.setItem("user_slot_id", user_slot_id)
            window.location = '../payment2'
        } else {
            swal('¡Atención!', 'Tu navegador no soporta Local Storage, por favor usa Google Chrome.', 'error')
        }
    }//vm.GoToPayment
